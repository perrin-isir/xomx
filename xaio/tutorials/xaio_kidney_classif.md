...
In the convention used by Scanpy, samples are stored in the raws of the
...
first sample. And `xd.X[:, 0]`, the first column, contains the expression levels of
...
so that the sum of the feature values is equal to `target_sum`.  It is a very basic 
...
Then, we save `xd` to the disk (as the file "xaio_kidney_classif.h5ad"
